...
| 2.6r1 | **Hardening: stack-based return arrow interleaving** — replace two-pass (all forwards then reversed returns) with a stack-based single-pass algorithm; return arrows now appear immediately after their corresponding call group rather than bunched at the end; fixes flat sequential traces | `[x]` | `Collections.reverse` removed; `ArrayDeque` stack added to `writeDiagram`; `emitReturn` extracted as helper; see `docs/features/feature-2.6-method-returns.md` revision history |
...
| 2.11 | **Argument-call tagging** — tag edges where the callee is called as an argument to another method (e.g. `f(g())`) with `is_arg_call = 1` in `call_edge`; suppress these ghost-sibling edges from `trace` output by default; `--include-arg-calls` flag opts back in | `[x]` | Detection: AST parent check distinguishes argument position from method-chain scope; `INSERT OR IGNORE` preserves first-occurrence tag; existing DBs migrated via `migrateAddColumn` (default `0` = safe); see `docs/features/feature-2.11-arg-call-tagging.md` |
...
*Last updated: 2026-04-20 (Feature 2.6r1 complete — stack-based render)*
